...
- 使用 Google Breakpad 库，程序崩溃时自动生成 minidump 文件到 `./crash` 目录，方便事后分析
...
- `zkApplication` 继承自 `QApplication`，**重写了 `qwsEventFilter()`**，用于拦截 Qt 窗口系统的底层事件——主要是**屏幕背光管理**：每次有用户操作（触摸/按键）就重置背光倒计时
- 四行编码设置确保整个程序在 **UTF-8** 下工作，避免中文乱码
