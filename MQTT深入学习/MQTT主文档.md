...
这个的作用是通知对方这一条消息的长度，比如变长编码是321，则说明还要向后读取321个字节。
最大能表示256MB，而MQTT不适合用来传输大文件，因此完全够用，不用考虑一条报文不够传递数据的问题。
