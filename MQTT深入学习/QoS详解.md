[[QoS与TCP协议对照]]其中涉及到四次握手，恰好一次的概念，联想到TCP协议 ，在这个文件中阐述了二者的关系

# MQTT QoS 详解 — 三级消息可靠性保证

QoS（Quality of Service）是 MQTT 协议定义的三级消息可靠性保证。从数据结构角度来理解——核心区别在于 **这条消息需要几次确认、确认的粒度是什么**。
...
它**不是一个协议**，而是 MQTT 协议**内部定义的一个字段/机制**——在 PUBLISH 报文的固定头里用 2 个 bit 表示（00=QoS0, 01=QoS1, 10=QoS2）。它只存在于 MQTT 协议体系内，控制的是"这条 MQTT 消息从发布者到订阅者之间怎么确认、怎么重传"。

叫"服务质量"容易让人以为和网络带宽、延迟保障有关（IP 层也有一个叫 QoS 的东西，完全不是一回事），MQTT 的 QoS 本质上就是**消息可靠性的三级开关**。
