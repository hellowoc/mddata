...
CA 搭建时（几年前）：
  CA 生成自己的公私钥对 → 私钥保密，公钥做成 root.crt

服务器部署时（几个月前）：
  服务器生成自己的公私钥对 → 私钥保密
  服务器把公钥放进 CSR → 发给 CA → CA 用自己私钥签名 → 返回 server.crt

设备出厂时：
  root.crt 预置到 /app/root.crt

设备运行时（现在）：
  TLS 握手 → 服务器发来 server.crt
  → 客户端用 root.crt 验证 → 从 server.crt 取服务器公钥
  → 生成临时密钥 → 用服务器公钥加密发给服务器
  → 只有持有服务器私钥的服务器能解开
