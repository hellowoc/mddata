...
按在 [[3 通道1-FPGA相机-主文档|主文档]] 中首次出现的顺序排列。
...
见 [[Socket编程基础#socket() — 创建 socket]]。

### AF_INET（§4-Step2）

见 [[Socket编程基础#AF_INET]]。
...
见 [[Socket编程基础#setsockopt() — 设置 socket 选项]]。
...
见 [[Socket编程基础#sockaddr_in — "地址+端口"结构体]]。

### memset()（§4-Step2）

见 [[C标准库函数#memset(ptr, value, n)]]。

### htons()（§4-Step2）

见 [[Socket编程基础#htons(x) / ntohs(x) — 字节序转换]]。

### inet_addr()（§4-Step2）

见 [[Socket编程基础#inet_addr(str) — 字符串 IP → 二进制]]。

### bind()（§4-Step2）

见 [[Socket编程基础#bind() — 绑定地址]]。

### inet_pton()（§4-Step2）

见 [[Socket编程基础#inet_pton(family, str, dst) — 新版 IP 字符串转换]]。
...
见 [[Qt核心类型#QThread]]。这里与通道2 不同——通道2 的 SendThread 在**构造函数**中调 `start()`，通道1 的 MyNetWorkThread 由外部显式调用 `udpthread->start()`。
...
见 [[Socket编程基础#select() — 检查 socket 是否有数据可读]]。
...
见 [[Socket编程基础#recvfrom() — 阻塞接收 UDP 数据]]。
