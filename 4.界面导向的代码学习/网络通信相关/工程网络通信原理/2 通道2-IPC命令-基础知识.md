...
按在 [[2 通道2-IPC命令-主文档|主文档]] 中首次出现的顺序排列。每个条目标明了在主文档的哪个章节首次出现。
...
见 [[2 通道2-IPC命令-主文档#3. 协议头结构|主文档 §3]]。定义在 `myudpthread.h:82-106`。
...
见 [[Qt网络类#QUdpSocket]]（Qt 版本）和 [[Socket编程基础#bind() — 绑定地址]]（BSD 版本）。
...
见 [[C标准库函数#malloc(n) / free(ptr)]]。

### QMap::key()（§5-Step4）

`QMap` 的方法。反向查找——通过 value 找第一个匹配的 key。见 [[Qt核心类型#QMap<Key, Value>]]。
...
UI 侧每秒从 `struIpcShare` 读取数据并刷新界面。见 [[Qt核心类型#QTimer]]。
